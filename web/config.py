...
class Config(object):
    ...
    AWS_GLACIER_VAULT = "xxx"

    # AWS S3 download parameters
    AWS_S3_SIGNED_DOWNLOAD_EXPIRATION = 60
    ...
    AWS_SNS_JOB_THAW_TOPIC = (
        f"arn:aws:sns:us-east-1:127134666975:{iam_username}_job_thaws"
    )
    ...
    AWS_DYNAMODB_ANNOTATIONS_TABLE_SECONDARY_INDEX = "user_id_index"
...
class ProductionConfig(Config):
    ...
    SSL_CERT_PATH = (
        os.environ["SSL_CERT_PATH"]
        if ("SSL_CERT_PATH" in os.environ)
        else "/usr/local/src/ssl/xxx.org.crt"
    )
    SSL_KEY_PATH = (
        os.environ["SSL_KEY_PATH"]
        if ("SSL_KEY_PATH" in os.environ)
        else "/usr/local/src/ssl/xxx.org.key"
    )
